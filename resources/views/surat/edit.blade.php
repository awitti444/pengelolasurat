...
@section('title', 'Edit')
@section('contents')
    <h1 class="mb-0">Edit Data Surat</h1>
    <hr />
    <form action="{{ route('surat.update', $letter->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="row mb-3">
            <div class="col mb-3">
                ...
                <input type="text" id="letter_perihal" name="letter_perihal" class="form-control" placeholder="Perihal" value="{{ $letter->letter_perihal }}">
            </div>
        </div>
        <div class="row mb-3">
            <div class="col mb-3">
                ...
                <select id="letter_type_id" name="letter_type_id" class="form-control">
                    @foreach($kualifikasiSuratOptions as $letterTypes)
                        <option value="{{ $letterTypes->id }}" @if($letter->letter_type_id == $letterTypes->id) selected @endif>{{ $letterTypes->name_type }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        <div class="row mb-3">
            <div class="col mb-3">
                ...
                <textarea class="form-control" id="des" name="content">{{ $letter->content }}</textarea>
            </div>
        </div>
        <div class="row mb-3">
            <div class="col mb-3">
        ...
        <div class="row mb-3">
            <div class="col mb-3">
                ...
                <select id="notulis_id" name="notulis_id" class="form-control" required>
                    @foreach($guru as $gurus)
                        <option value="{{ $gurus->id }}" @if($letter->notulis_id == $gurus->id) selected @endif>{{ $gurus->name }}</option>
                    @endforeach
                </select>
            </div>
        </div>
